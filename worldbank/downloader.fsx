...
let indicatorsMeta =  
  ...
  data |> Array.map (fun i ->
    ...
    { Index = 0us; DataPoints = -1; ID = i.Id; Name = i.Name; Source = i.Source.Value; 
      SourceNote = defaultArg i.SourceNote ""; SourceOrg = defaultArg i.SourceOrganization ""; 
      Topics = [ for t in i.Topics -> t.Value ] }) |> dict
...
let cont = account.CreateCloudBlobClient().GetContainerReference("worldbank")
cont.CreateIfNotExists()

let uploadFile file = 
  let blob = cont.GetBlockBlobReference(file)
  blob.UploadFromFile(__SOURCE_DIRECTORY__ + "\\cache\\" + file, FileMode.Open)

uploadFile "countries.json"
uploadFile "indicators.json"
uploadFile "years.json"
uploadFile "worldbank.dat"
